...
function applyMove(x, y, move) {
  ...
  else
  {
	...
	state.board[move.landings[index].y][move.landings[index].y] = state.board[y, x];
	state.board[y, x] = null;
  }
}
...
function checkForVictory(){
  ...
  for(y = 0; y < 10; y++){
	for(x = 0; x < 10; x++){
		...
		else if(state.board[y][x] === 'b' || state.board[y][x] === 'bk'){
			bCount++;
		}
	}  
  }
...
function printBoard(){
  ...
  for(y = 0; y < 10; y++){
	for(x = 0; x < 10; x++){
		...
		else if(state.board[y][x] === 'wk'){
			console.print('wk');
		}
		else if(state.board[y][x] === 'b'){
			console.print('b.');
		}
		else if(state.board[y][x] === 'bk'){
			console.print('bk');
		}
		else{
			console.print('..');
